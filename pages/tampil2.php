    ...
    for($list_day = 1; $list_day <= $days_in_month; $list_day++):
        ...
        $calendar.= '<td class="calendar-day '.$currentday.'">';
            ...
            $calendar.= '<div class="day-number">'.$showtoday.'<br></div>';
                        $no = '0';
                        $sumpendapatan='0';
                        $result = mysqli_query($con,"SELECT tgl,sum(masuk) as masuk,sum(keluar) as keluar FROM cashflow where month(tgl)='$month' and year(tgl)='$year' and day(tgl)='$showtoday' group by tgl");
