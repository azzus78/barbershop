            ...
            <td>
                <table border='1'>
                    <thead>
                        ...
                        <?php
                        $no = '0';
                        $result = mysqli_query($con,"SELECT user,count(tgl) as kehadiran FROM absensi where  durasi>'00:02:00' and month(tgl)='$bulan' and year(tgl)='$tahun' and sudahpulang='1' group by user");
                        while($row=mysqli_fetch_array($result))
                        {
                            ...
                            $kehadiran =$row['kehadiran'];
                                ...
                                $jumshift=mysqli_query($con,"SELECT count(nama) as jumlah_shift FROM jadwal where nama='$user' and bulan='$bulan' and tahun='$tahun' group by nama");
                                $data=mysqli_fetch_array($jumshift);
                                mysqli_free_result($jumshift);
                                if (empty($data)) {
                                    $jumlah_shift='1';
                            ...
                            <?php 
                            $result2 = mysqli_query($con,"SELECT user,count(telat) as telat FROM absensi where telat>'00:00:00' and user='$user' and sudahpulang='1' and month(tgl)='$bulan' and year(tgl)='$tahun'");
                ...
                <table border="1"  width="100%">
                    ...
                    <tbody>
                        <?php
                            ...
                            $result = mysqli_query($con,"SELECT * FROM absensi where month(tgl)='$bulan' and year(tgl)='$tahun' and sudahpulang='1' ORDER BY user");
                            while($row =mysqli_fetch_array($result)) {
                            $no++;
                            $tanggal=$row['tgl'];
                            $telat = $row['telat'];
