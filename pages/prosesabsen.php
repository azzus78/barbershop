...
if (!empty($_POST["proses"])) {
	...
	switch($proses)
	{
		case 'jadwal':
			...
			foreach ($_POST as $kunci => $isi)
			{
				...
				if ($pos) {
					...
					$nama = $pisah['name'];
					$shift = $pisah['shift'];
					$result=mysqli_query($con,"INSERT INTO jadwal(tgl,nama,shift,bulan,tahun) 
		...
		case 'datang':
			...
			if (empty ($username) or empty ($password)){
				...
				if ($password == $pengguna['password']) {
					...
					$user = $jenengawal[0];
					$result=mysqli_query($con,"SELECT * FROM absensi WHERE user = '$user' and sudahpulang='0'");
					...
					if (!empty($data)) {
						...
						if ($shift=='4') {
							...
							if ($ganjil % 2 == 1) {
							$result=mysqli_query($con,"INSERT INTO absensi(tgl,datang,user,shift,telat) 
		...
		case 'pulang':
			...
			<?php
				...
				$result=mysqli_query($con,"SELECT datang FROM absensi WHERE user='$user' and sudahpulang='0'");
				$absensi = mysqli_fetch_array($result);
				mysqli_free_result($result);	
				...
				$update=mysqli_query($con,"UPDATE absensi set pulang='$jampulang',sudahpulang='1',durasi='$durasi' where user='$user' and sudahpulang='0' ");
				header("Location: index.php?cp=presensi");
				}else{?>
					<script language="javascript">
						alert("Username dan Password tidak Cucok!!");
    ...
    switch ($proses) {
    	case 'jadwal':
    		$id=$_GET['idjadwal'];
    		//echo $id;
    		$delete = mysqli_query($con,"DELETE FROM jadwal Where idjadwal='$id'") or die(mysqli_error()); 
    		//$update=mysqli_query($con,"update jadwal set shift='2' where idjadwal='$id'");
